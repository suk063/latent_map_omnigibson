...
class _InferLevel(nn.Module):
    ...
    def query(self, pts, mark_accessed: bool = False):
        with torch.no_grad():
            if self.one_to_one:
                q = (pts - self.smin) / self.res
                mask = ((q < 0) | (q >= self.grid_shape)).any(dim=-1)
...
class VoxelHashTable(nn.Module):
    ...
    def __init__(
        ...
        if mode == "train":
            ...
            # If pre-trained data is provided, load the state_dict.
            # This is simpler and more robust as it uses PyTorch's built-in mechanism.
            if sparse_data is not None:
                state_dict = sparse_data.get('state_dict', sparse_data)
    ...
    def load_sparse(path, device="cuda:0"):
        ...
        grid = VoxelHashTable(mode="infer", sparse_data=sparse, device=device)
